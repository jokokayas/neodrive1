...
function config($key)
{
    $file = file_get_contents("base/data/setting/config.json");
...
{
    $curl = curl_init();
    $opts = [CURLOPT_URL => 'https://api.vidcloud.co/v1/remotedl/add?login=' . $login . '&key=' . $key . '&url=' . $url . '', CURLOPT_RETURNTRANSFER => true, ];
    curl_setopt_array($curl, $opts);
    $response = json_decode(curl_exec($curl) , true);
    return $response;
}
...
{
    $curl = curl_init();
    $opts = [CURLOPT_URL => 'https://api.vidcloud.co/v1/remotedl/status?login=' . $login . '&key=' . $key . '&id=' . $id . '', CURLOPT_RETURNTRANSFER => true, ];
    curl_setopt_array($curl, $opts);
    $response = json_decode(curl_exec($curl) , true);
    return $response;
}
...
{
    $file = "base/data/setting/down.json";
    if (!file_exists($file))
    {
...
{
    $file = "base/data/main/user/$user/cache/down.json";
    ...
    if (!file_exists($file))
    {
        file_put_contents("base/data/main/user/$user/cache/down.json", null);
    }
    $get = json_decode(file_get_contents($file) , true);
    $download = $get[download];
    if ($download == null)
